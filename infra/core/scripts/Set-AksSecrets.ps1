...
try {
    ...
    # $encryptSecretErrorsIgnore = $encryptSecrets | Select-String 'Error from server (Conflict)'
    $encryptSecretWarning = $encryptSecrets | Select-String 'the object has been modified; please apply your changes to the latest version and try again'
    $encryptSecretErrors = $encryptSecrets | Select-String Error
    if ($NULL -ne $encryptSecretWarning) {
        Write-Host "The secret has been modified by another process."
    } elseif ($NULL -ne $encryptSecretErrors) {
